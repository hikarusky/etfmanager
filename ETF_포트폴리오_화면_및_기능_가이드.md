...
**Q. 상단 바의 User ID는 어떻게 바꾸고 다른 계좌를 보나요?**  
A. 화면 상단 헤더 바로 아래의 `User ID:` 드롭다운을 클릭하면 DB에 등록된 모든 User ID와 각 계정의 계좌수/종목수가 목록으로 나타납니다. 원하는 계정을 선택하면 해당 사용자의 포트폴리오와 계좌가 즉시 조회됩니다.

**Q. 테스트용으로 만들었거나 불필요한 User ID를 삭제하고 싶어요.**  
A. 상단 User ID 바 우측의 설정(⚙) 아이콘을 눌러 [모달 6] 계정 설정 창을 엽니다. 하단의 'DB 등록 계정에서 선택' 드롭다운에서 삭제할 계정을 선택한 후 **`[삭제]`** 버튼을 누르면 DB에서 안전하게 영구 삭제됩니다. (단, 기본 계정인 `hikarusky`는 데이터 보호를 위해 삭제할 수 없습니다.)

**Q. 퇴직연금(DC/IRP) 계좌에서 미국채나 국채 ETF는 왜 '(NO위험자산)'으로 표기되나요?**  
A. 퇴직연금 감독규정상 주식형 등 위험자산은 계좌 평가금액의 최대 70%까지만 담을 수 있으며, 최소 30%는 국채·미국채·채권·TDF 등 안전자산으로 채워야 합니다. 본 서비스는 종목명에 '채권', '국채', '미국채', 'TDF'가 포함된 종목(예: `ACE 미국30년국채액티브`, `TIGER 미국채10년선물` 등)을 자동으로 'NO위험자산(안전자산)'으로 인식하여 비중을 정확하게 분리 계산해 드립니다.
