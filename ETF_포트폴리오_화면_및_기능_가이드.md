...
- **주요 특징**:
  ...
  4. **퇴직연금(DC/IRP) 위험자산 한도 진단**: 주식형 위험자산(70% 한도)과 채권·TDF 등 안전자산 비중을 실시간 분리 집계
...
- **동작**:
  ...
  - **퇴직연금(DC / IRP) 계좌 전용 위험자산 진단 배너**:
    ...
    - 종목명에 채권이나 TDF가 포함된 종목을 자동으로 'NO위험자산'으로 판별하고, **위험자산 비중(%)과 안전자산 비중(%)**을 실시간 계산하여 배너에 표시합니다.
